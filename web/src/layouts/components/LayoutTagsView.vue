<template>
  <div :class="['layout-tabs', tabsStyleClass]">
    <button
      v-if="hasOverflow"
      type="button"
      class="layout-tabs__scroll-arrow"
      aria-label="向左滚动页签"
      title="向左滚动页签"
    ...
    >
      <div ref="tabsListRef" class="layout-tabs__list">
        <div
...
const updateScrollState = () => {
  ...
  if (!wrapper || !tabsListRef.value) return;

  // Include both arrows and their gaps when measuring the available space.
  const availableWidth = wrapper.clientWidth + (hasOverflow.value ? 68 : 0);
  hasOverflow.value = tabsListRef.value.scrollWidth > availableWidth + 1;
  canScrollLeft.value = wrapper.scrollLeft > 1;
  canScrollRight.value = wrapper.scrollLeft + wrapper.clientWidth < wrapper.scrollWidth - 1;
};

const scrollTabsRight = () => {
  closeContextMenu();
  const wrapper = scrollbarRef.value?.wrapRef as HTMLElement | undefined;
  wrapper?.scrollBy({ left: Math.max(wrapper.clientWidth * 0.8, 120), behavior: "smooth" });
};
...
const scrollActiveTagIntoView = async () => {
  await nextTick();
  updateScrollState();
  // Wait for the overflow arrows to take their space before measuring the viewport.
  await nextTick();
  const wrapper = scrollbarRef.value?.wrapRef as HTMLElement | undefined;
  const activeTag = tabsListRef.value?.querySelector<HTMLElement>(".is-active");
  if (!wrapper || !activeTag) return;

  const viewport = wrapper.getBoundingClientRect();
  const tagBounds = activeTag.getBoundingClientRect();
  let offset = 0;
  if (tagBounds.width > wrapper.clientWidth || tagBounds.left < viewport.left) {
    offset = tagBounds.left - viewport.left;
  } else if (tagBounds.right > viewport.right) {
    offset = tagBounds.right - viewport.right;
  }
...
watch([() => route.fullPath, visitedViews, tabsStyleClass], scrollActiveTagIntoView, {
  deep: true,
  flush: "post",
});
...
onMounted(() => {
  ...
  const wrapper = scrollbarRef.value?.wrapRef;
  if (wrapper) tabsResizeObserver.observe(wrapper);
  if (tabsListRef.value) tabsResizeObserver.observe(tabsListRef.value);
  scrollActiveTagIntoView();
});
